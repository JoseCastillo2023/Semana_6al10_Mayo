...
const App = () => {
  ...
  const agregarCliente = () => {
    ...
    const nuevoCliente = { id: clientes.length + 1, nombre: nombre, fechaReserva: fechaReserva, reserva: reserva };
  ...
  // Función que se ha seleccionado un cliente
  const displayMessage = () => {
    Alert.alert("Cliente seleccionado.");
  };

  // Agregame un boton que vaya a otro lugar en expo reactive?
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          ...
          >
            ...
            <Text style={styles.clienteNombre}>Cantidad Reservada: {item.reserva} </Text>
            <Text style={styles.clienteFecha}>
              Fecha de Reserva: {item.fechaReserva.toDateString()}
            </Text>
            <Button title='Eliminar cliente' style={styles.btnEliminar} onPress={() => eliminarCliente(item.id)} color="red"></Button>
          </TouchableOpacity>
        )}
